...
contract Trypto is ERC721, ERC721URIStorage, Ownable, AutomationCompatibleInterface {
    ...
    // For Event Interval (10 minutes)
    uint internal eventTimeInterval;
    ...
    //     result = randomResult * 100; //2 decimal places
    ...
    // every 10 minutes, pick random tokenId using VRF(0 to _tokenIdCounter.current())
    // then Search the address who own the tokenId.
    // then Give Present to the address.
    function eventDrop() public {
        if(eventTimeInterval >= 10) {
            //WORKING ON IT
        }
